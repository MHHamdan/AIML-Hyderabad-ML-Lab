...
def get_expression(img_str):
    ...
    # YOUR CODE HERE, return expression using your model
        image_1 = trnscm(face).unsqueeze(0)
        ...
        model.fc = nn.Sequential(nn.Linear(2048, 256),
                        ...
                        nn.ReLU(),
                        nn.Linear(100, 7))
        feature_checkpoint = torch.load('/home/b15/b15h3g12/Hackathon-setup/resnet50_model.t7', map_location=device)
    model.load_state_dict(feature_checkpoint['net_dict'])
        output = model(image_1)
        size_ = output.size()
        output_ = output.view(size_[0], 4)
